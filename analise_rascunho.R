...
df_rides <- read_delim("dados_mais_bike/dados_maisbike_2018.csv", ";")
...
# Manipulação dos dados em 'df_rides' ----------------------------------

# criação da variavel duracao_viagem
df_rides <- df_rides %>% 
  mutate(duracao_viagem = (time_end - time_start)) %>% 
...
df_estacoes <- tibble(estacao = unique(df_rides$station_start),
                      estacao2 = unique(df_rides$station_start))
...
df_estacoes <- df_estacoes %>% 
  left_join(n_partidas, by = c("estacao" = "station_start"))

n_devolucoes <- df_rides %>% 
  group_by(station_end) %>% 
  summarise(n_devolucoes = n())

df_estacoes <- df_estacoes %>% 
  left_join(n_devolucoes, by = c("estacao" = "station_end"))
...
df_rides %>% 
  mutate(hora_partida = hour(time_start)) %>% 
  filter(station_start == "41 - Instituto de Artes") %>% 
  ggplot()+
  geom_bar(aes(x = hora_partida))+
  scale_x_continuous(breaks = seq(0,23))

df_rides %>% 
  mutate(hora_devolucao = hour(time_end)) %>% 
  filter(station_start == "41 - Instituto de Artes") %>% 
  ggplot()+
...
df_rides %>% 
  group_by(station_start, ride_date) %>% 
  summarise(n_viagens = n()) %>% 
  filter(station_start == "41 - Instituto de Artes") %>% 
  ggplot()+
  geom_line(aes(x = ride_date, y = n_viagens), stat = "identity")+
...
df_rides <- df_rides %>% left_join(select(df_estacoes, estacao, lat_inicial = lat, lon_inicial = lon), by = c("station_start" = "estacao")) %>% 
  left_join(select(df_estacoes, estacao, lat_final = lat, lon_final = lon), by = c("station_end" = "estacao"))
